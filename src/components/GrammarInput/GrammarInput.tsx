...
type GrammarInputParam = {
  input: GRAMMAR_INPUT,
  handleInput: (id: string, e: Event) => void,
  handleRemove: (id: string) => void
}

export default function GrammarInput({input, handleInput, handleRemove}: GrammarInputParam) {
  const NONTERMINAL_PATTERN = /^[A-Z]+$/
  const TERMINAL_PATTERN = /^".+"$/
  let lhError = input.LH.trim() ? !NONTERMINAL_PATTERN.test(input.LH.trim()) : false;
  let rhError = input.RH.trim() ? !input.RH.trim().split('|').every((production)=> TERMINAL_PATTERN.test(production.trim()) || NONTERMINAL_PATTERN.test(production.trim())) : false; 
  return (
    <div className='flex'>
      ...
      <button className='bg-red-400 py-1 px-1' onClick={()=>handleRemove(input.id)}>
